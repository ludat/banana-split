  ...
  getResumen totalPago repartija =
    if
      | null repartija.items ->
          NetosIncomputables (Just totalPorItems) $
            ErrorResumen (Just "No hay items para repartir.") []
      | totalPorItems /= totalPago ->
          NetosIncomputables (Just totalPorItems) $
            ErrorResumen (Just $ "El total de items debería ser igual al total del pago pero es: " <> show totalPorItems <> " en vez de " <> show totalPago) []
      | null repartija.claims ->
          NetosIncomputables (Just totalPorItems) $
            ErrorResumen (Just "Nadie reclamo ningun item.") []
      | totalReclamado /= totalPago ->
          NetosIncomputables (Just totalPorItems) $
            ErrorResumen (Just $ "El total de reclamado debería ser igual al total del pago pero es: " <> show totalReclamado <> " en vez de " <> show totalPago) []
      | otherwise ->
          ResumenNetos (Just totalPorItems) netosReclamados
    where
