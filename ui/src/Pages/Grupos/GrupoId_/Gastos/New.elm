module Pages.Grupos.GrupoId_.Gastos.New exposing (Model, Msg, ReceiptReadingState, Section(..), andThenSendWarningOnExit, page, subscriptions, update, validatePago, validatePagoInSection, view, waitAndCheckNecessaryData)
    ...
    case msg of
        ...
        Cancel ->
            ...
            , Effect.batch
                ...
                , Effect.pushRoutePath <| Path.Grupos_GrupoId__Gastos { grupoId = model.grupoId }
                ]
            )
...
{-| Navega a la lista de gastos abriendo el modal de detalle del gasto dado (la
página de gastos abre ese modal cuando ve `?gasto=<id>` en la URL).
-}
irAlPagoEnLista : ULID -> ULID -> Effect msg
irAlPagoEnLista grupoId pagoId =
    Effect.pushRoute
        { path = Path.Grupos_GrupoId__Gastos { grupoId = grupoId }
        , query = Dict.singleton "gasto" pagoId
        , hash = Nothing
        }
